...
def add_poligono(nuevoPoligono):
    PoligonoInsertado = collection.insert_one(nuevoPoligono)
    print("Poligono Insertado")


def delete_poligono(nuevoPoligono):
        print ("delete")
...
def on_message(client_mqtt, userdata, message):
    ...
    if peticion == "mongo/mobileApp/getPoligonos":
        ...
        add_poligono(nuevopoligonoprueba)
    elif peticion =="mongo/mobileApp/deletePoligonos":

        nuevopoligonoprueba = json.loads(message.payload.decode("utf-8"))
        delete_poligono(nuevopoligonoprueba)


client_mqtt = mqtt.Client("MongoDBService")
client_mqtt.on_message = on_message
client_mqtt.connect("localhost", 8000)
