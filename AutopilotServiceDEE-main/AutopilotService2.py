...
def add_prueba_poligono(poligonos: PruebaPoligonosEsquema):
    try:
        nuevoPoligono = PruebaPoligonos(Jugadores=poligonos.Jugadores,Poligonos=poligonos.Poligonos)
        nuevoPoligono.save()
        return {'success':True,'message':'poligono registrado'}
...
def process_message(message, client):
    ...
    if command =="GetObstaculos":
        external_client.publish(sending_topic + "/ReadOstaculos", json.dumps(get_prueba_poligonos()))

    if command == "AddObstaculos":

        add_prueba_poligono(json.loads(message.payload.decode()))
